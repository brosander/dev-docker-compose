...
unzip "$NIFI_TOOLKIT" -d "$BASE_DIR/target"
...
unzip -p "$NIFI_ARCHIVE" "$NIFI_CONF_DIR/authorizers.xml" > "$BASE_DIR/target/base/authorizers.xml"
...
cp "$BASE_DIR/target/base/authorizers.xml" "$BASE_DIR/target/basenode/authorizers.xml"
sed -i.bak 's/<property name="Initial Admin Identity"><\/property>/<property name="Initial Admin Identity">CN=admin, OU=NIFI<\/property>/g' "$BASE_DIR/target/basenode/authorizers.xml"
sed -i.bak 's/<\/authorizer>/<!--<\/authorizer>/g' "$BASE_DIR/target/basenode/authorizers.xml"
sed -i.bak 's/<\/authorizers>/<\/authorizers>-->/g' "$BASE_DIR/target/basenode/authorizers.xml"

echo "" >> "$BASE_DIR/target/basenode/authorizers.xml"

for i in $(seq 1 $NUM_NODES); do
  echo "        <property name=\"Node Identity $i\">CN=node$i.nifi, OU=NIFI</property>" >> "$BASE_DIR/target/basenode/authorizers.xml"
done

echo "    </authorizer>" >> "$BASE_DIR/target/basenode/authorizers.xml"
echo "</authorizers>" >> "$BASE_DIR/target/basenode/authorizers.xml"

TLS_TOOLKIT_SH="$(find target/nifi-toolkit-* -name tls-toolkit.sh)"

"$TLS_TOOLKIT_SH" standalone -C "CN=admin, OU=NIFI" -o "$BASE_DIR/target"

for i in $(seq 1 $NUM_NODES); do
  echo "Configuring node$i"
  "$TLS_TOOLKIT_SH" standalone -f "$BASE_DIR/target/basenode/nifi.properties" -n node$i.nifi -o "$BASE_DIR/target"
  cp -rn "$BASE_DIR/target/basenode/"* "$BASE_DIR/target/node$i.nifi/"
  ...
  mkdir -p "$BASE_DIR/target/node$i.nifi/state/zookeeper"
  echo "$i" > "$BASE_DIR/target/node$i.nifi/state/zookeeper/myid"
done
...
for i in $(seq 1 $NUM_NODES); do
  ...
  echo "      - ./node$i.nifi:/opt/nifi-conf" >> "$BASE_DIR/target/docker-compose.yml"
